load --os $HOME/Desktop/myexpos/spl/spl_progs/final_os_startup.xsm
load --init $HOME/Desktop/myexpos/expl/sample/shell_2.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/parent.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/sem22.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/pid.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/llist.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/child.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/fork21.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/test.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/conMS.xsm
load --exec $HOME/Desktop/myexpos/expl/sample/seqMS.xsm
load --idle $HOME/Desktop/myexpos/expl/sample/idle.xsm
load --exhandler $HOME/Desktop/myexpos/spl/spl_progs/exception_handler.xsm
load --library $HOME/Desktop/myexpos/expl/library.lib
load --int=timer $HOME/Desktop/myexpos/spl/spl_progs/timer_handler.xsm
load --int=console $HOME/Desktop/myexpos/spl/spl_progs/console_interrupt.xsm
load --int=disk $HOME/Desktop/myexpos/spl/spl_progs/disk_interrupt.xsm
load --int=6 $HOME/Desktop/myexpos/spl/spl_progs/int_6.xsm
load --int=7 $HOME/Desktop/myexpos/spl/spl_progs/sample_int_7.xsm
load --int=8 $HOME/Desktop/myexpos/spl/spl_progs/int_8.xsm
load --int=9 $HOME/Desktop/myexpos/spl/spl_progs/int_9.xsm
load --int=10 $HOME/Desktop/myexpos/spl/spl_progs/int_10.xsm
load --int=11 $HOME/Desktop/myexpos/spl/spl_progs/int_11.xsm
load --int=13 $HOME/Desktop/myexpos/spl/spl_progs/int_13.xsm
load --int=14 $HOME/Desktop/myexpos/spl/spl_progs/int_14.xsm
load --int=15 $HOME/Desktop/myexpos/spl/spl_progs/haltprog.xsm
load --module 0 $HOME/Desktop/myexpos/spl/spl_progs/module_0_stage15.xsm
load --module 1 $HOME/Desktop/myexpos/spl/spl_progs/module_1.xsm
load --module 2 $HOME/Desktop/myexpos/spl/spl_progs/module_2.xsm
load --module 4 $HOME/Desktop/myexpos/spl/spl_progs/module_4_stage15.xsm
load --module 5 $HOME/Desktop/myexpos/spl/spl_progs/module_5.xsm
load --module 7 $HOME/Desktop/myexpos/spl/spl_progs/module_7.xsm
